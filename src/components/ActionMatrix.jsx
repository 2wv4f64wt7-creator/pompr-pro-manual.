/* POMPR-PRO COMPONENT: ACTION MATRIX
   VERSION: V12.4 (COLOR-MAPPING RESTORATION)
   ARCHITECT NOTE: Anchored the UI in Action Green (#10b981) while preserving the card heatmap.
*/
...
const UTILITY_ACTIONS = [
  ...
  { id: 'U_REF_HEAD', category: 'UTILITY', intensity: 5, name: '3D-HEAD', desc: '3-panel head reference sheet with front view, side profile, and rear view, consistent proportions, neutral expression, white background, even lighting, high detail clarity, no motion' },
  { id: 'U_REF_BODY', category: 'UTILITY', intensity: 7, name: '3D-BODY', desc: 'full body character reference sheet with front, side, three-quarter, and back views, identical scale, neutral upright stance, white background, even lighting, high detail clarity, no motion' },
...
const ACTION_GREEN = '#10b981'; // The persistent brand color for Actions
...
const ActionMatrix = ({ actions = [], onSelectAction }) => {
  ...
  const [intensity, setIntensity] = useState(5); // Default to middle intensity
  ...
  // --- STYLES ---
  const styles = {
    // ARCHITECT FIX: Top border changed to Action Green to anchor the zone
    ...
    filterBtn: (active) => ({ 
      ...
      // ARCHITECT FIX: Active tabs are now firmly Action Green
      background: active ? ACTION_GREEN : '#222', 
    ...
    // Heatmap card logic perfectly preserved
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.header}>
        {/* ARCHITECT FIX: Title is persistent Green, Intensity Label is dynamic Heatmap */}
        <span style={{ color: ACTION_GREEN }}>
          {activeCategory === 'UTILITY' ? 'UTILITY COMMANDS' : 'ACTION MATRIX'}
        </span>
        <span style={{ color: getColorByIntensity(intensity) }}>
          {activeCategory === 'UTILITY' ? 'GLOBAL' : `Lvl ${intensity}: ${getIntensityLabel(intensity)}`}
        </span>
      </div>
