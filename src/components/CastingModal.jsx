// -------------------------------------------------------------------
// FILE: src/components/CastingModal.jsx
// VERSION: 11.0 (UNIFIED EXPORT UPDATE)
// -------------------------------------------------------------------
...
export default function CastingModal({ onClose, onSave }) {
  ...
  const handleUnifiedSave = () => {
    ...
    link.setAttribute("download", `${(formData.name || 'Character').replace(/\s+/g, '_')}_Card.json`);
    link.click();
    ...
    <div style={{position:'fixed', top:0, left:0, width:'100vw', height:'100vh', background:'rgba(0,0,0,0.95)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1000, padding:'40px'}}>
      <div style={{background:'#0D0D0D', border:'1px solid #333', width:'100%', maxWidth:'850px', borderRadius:'15px', padding:'40px', maxHeight:'90vh', overflowY:'auto'}}>
        ...
        <div style={{marginBottom:'25px', borderTop:'1px solid #222', paddingTop:'20px'}}>
             ...
             <input placeholder="e.g. https://s.mj.run/..." value={formData.refUrl} onChange={(e) => setFormData({...formData, refUrl: e.target.value})} style={{...inputStyle, width: '100%', borderColor: formData.refUrl ? '#0070f3' : '#333'}} />
        </div>
        ...
        <div style={{display:'flex', gap:'15px'}}>
          <button onClick={handleUnifiedSave} style={{flex:3, background:'#00FF88', color:'#000', padding:'18px', fontWeight:'900', border:'none', cursor:'pointer', borderRadius:'4px', letterSpacing:'1px'}}>SAVE & EXPORT .JSON CARD</button>
