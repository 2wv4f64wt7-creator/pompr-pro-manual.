// -------------------------------------------------------------------
// FILE: App.jsx
// VERSION: 11.15 (RUNTIME CRASH FIX)
// ARCHITECT NOTE: Scrubbed all ghost references to renderParams.
// -------------------------------------------------------------------
...
const baseInteractions =
  reelData?.interactions?.length > 0
    ? reelData.interactions
    : SAFE_INTERACTIONS;
...
export default function App() {
  ...
  const [customCharacters, setCustomCharacters] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem('PPRO_CUSTOM_CAST') || '[]');
    } catch {
      return [];
    }
  });
  const [customScenes, setCustomScenes] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem('PPRO_CUSTOM_SCENES') || '[]');
    } catch {
      return [];
    }
  });
  const [customActions, setCustomActions] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem('PPRO_CUSTOM_ACTIONS') || '[]');
    } catch {
      return [];
    }
  });
  const [customMeta, setCustomMeta] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem('PPRO_CUSTOM_META')) || null;
    } catch {
      return null;
    }
  });
  ...
  const [characters, setCharacters] = useState(
    [...customCharacters, ...reelData.characters].filter(Boolean)
  );
  const [scenes, setScenes] = useState(
    [...customScenes, ...reelData.scenes].filter(Boolean)
  );
  const [actions, setActions] = useState(
    [...customActions, ...reelData.actions].filter(Boolean)
  );
  ...
  useEffect(() => {
    if (customMeta)
      localStorage.setItem('PPRO_CUSTOM_META', JSON.stringify(customMeta));
  ...
  const getDynamicPrompt = () => {
    ...
    const cineText = scene
      ? `CINEMATOGRAPHY: ${scene.lighting}, Cinematic Lens.`
      : '';

    const activeMeta = customMeta || reelData?.meta || {};
    const globalStyleText = activeMeta.global_style
      ? `STYLE: ${activeMeta.global_style}.`
      : '';
  ...
  const triggerRandomix = () => {
    ...
    if (Math.random() > 0.2)
      setActor1(characters[Math.floor(Math.random() * characters.length)]);
    else setActor1(null);
    setActor2(null);
  ...
  const exportAsset = (item) => {
    ...
    downloadAnchorNode.setAttribute(
      'download',
      `${item.name.replace(/\s+/g, '_')}_Card.json`
    );
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            The Director's Console is a professional production environment.
            Please open POMPR on a desktop or tablet for the full widescreen
            experience.
          </p>
        </div>
      </div>
    ...
    >
      ...
      <div style={{ ...laneStyle, left: '0%', width: '30%' }}>
        <ReelColumn
          ...
          onCreateClick={() => setShowSceneModal(true)}
          onExport={exportAsset}
          onSelect={(s) => {
            setIsManual(false);
            setScene(s.id === scene?.id ? null : s);
          }}
        />
      </div>
      ...
      <div style={{ ...laneStyle, left: '30%', width: '30%' }}>
        <ReelColumn
          ...
          colorTheme="orange"
          showCreateButton={true}
          onCreateClick={() => setShowCastModal(true)}
          onExport={exportAsset}
          onSelect={(char) => {
            setIsManual(false);
            if (activeSlot === 1) {
              if (actor1 && actor1.id === char.id) setActor1(null);
              else setActor1(char);
            } else {
              if (actor2 && actor2.id === char.id) setActor2(null);
              else setActor2(char);
            }
          }}
          headerSlot={
            ...
            >
              <button
                ...
                style={{
                  ...
                  fontSize: '10px',
                  padding: '8px',
                  background: activeSlot === 1 ? '#ff8a00' : 'transparent',
                  color: activeSlot === 1 ? 'white' : '#888',
                  border: 'none',
                  cursor: 'pointer',
                  fontWeight: '900',
                  borderRadius: '2px',
              ...
              >
                {actor1 ? 'ACTOR 1' : 'NO ACTOR'}
              </button>
              <button
                onClick={() => {
                  setActiveSlot(2);
                  if (!actor2 && actor1) setActor2(reelData.characters[1]);
                }}
                style={{
                  flex: 1,
                  fontSize: '10px',
                  padding: '8px',
                  background: activeSlot === 2 ? '#ff8a00' : 'transparent',
                  color: activeSlot === 2 ? 'white' : '#888',
                  border: 'none',
                  cursor: 'pointer',
                  fontWeight: '900',
                  borderRadius: '2px',
      ...
      <div style={{ ...laneStyle, left: '60%', width: '40%' }}>
        <ScriptConsole
          ...
          dynamicPrompt={dp}
          fullDynamicString={fullDynamicString}
          actions={actions}
          action={action}
          setAction={(a) => {
            setAction(a);
      ...
      <div style={{ position: 'absolute', zIndex: 100 }}>
        {showCastModal && (
          <CastingModal
            ...
            onSave={(c) => smartSetCustomCharacters([c])}
          />
        )}
        {showSceneModal && (
          <SceneBuilderModal
            onClose={() => setShowSceneModal(false)}
            onSave={(s) => smartSetCustomScenes([s])}
          />
        )}
        {showVault && (
          <TechVaultModal
